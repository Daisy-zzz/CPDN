...
class MODEL(torch.nn.Module):
    def __init__(self):
        ...
        # self.proj_v_5 = nn.Sequential(
        #     nn.Conv1d(VISUAL_DIM, self.d_l, kernel_size=5, padding=2, stride=1, bias=False),
        #     nn.ReLU(inplace=True)
        # )
        #
        # self.proj_v_7 = nn.Sequential(
        #     nn.Conv1d(VISUAL_DIM, self.d_l, kernel_size=7, padding=2, stride=1, bias=False),
        #     nn.ReLU(inplace=True)
        # )
        ...
        # self.fusion_category_0 = nn.Sequential(
        #     nn.Linear(self.d_l, CAT_EMB),
        #     nn.ReLU()
        # )
        # self.fusion_category_1 = fusion_category(self.d_l, SUBCAT_EMB)
        # self.fusion_category_2 = fusion_category(self.d_l, CONCEPT_EMB)
        # 坑爹，加了初始化反而效果不好
        # self.init_weight()
    ...
    def forward(
        ...
        # visual_5 = self.proj_v_5(visual.transpose(1, 2)).transpose(1, 2)
        # visual_7 = self.proj_v_7(visual.transpose(1, 2)).transpose(1, 2)
        ...
        # cat_emb, cat_loss = self.fusion_category_0(visual_7)
        # subcat_emb, subcat_loss = self.fusion_category_1(visual_7, visual_5, category[:, :, 1])
        # concept_emb, concept_loss = self.fusion_category_2(subcat_emb, visual_emb, category[:, :, 2])
    ...
    def test(
        ...
        concept = self.concept_emb(category[:, :, 2])
        category_emb = self.fusion_category(torch.cat([cat, subcat, concept], dim=-1))
        #category_emb = F.relu(self.fusion_category_2(self.fusion_category_1(cat, subcat), concept))
...
class fusion_category(nn.Module):
    def __init__(self, d_l, out):
        super(fusion_category, self).__init__()
        self.d_l = d_l
        self.linear_last = nn.Linear(self.d_l, self.d_l)
        self.linear_this = nn.Linear(self.d_l, self.d_l)
        self.linear_out = nn.Sequential(
            nn.Linear(self.d_l * 2, out),
            nn.ReLU()
        )
        self.loss = nn.CrossEntropyLoss()
        self.sigmoid = nn.Sigmoid()

    def forward(self, f_last, f_this, label):
        f_this_gate = self.sigmoid(self.linear_last(f_last) + self.linear_this(f_this))
        f_last_gate = torch.mul(f_last, f_this_gate)
        f_this_fusion = torch.cat([f_this, f_last_gate], dim=-1)
        f_out = self.linear_out(f_this_fusion)
        loss = self.loss(f_out.transpose(), label)
        return f_this_fusion, loss
...
class att_lstm(nn.Module):
    def __init__(self, dim):
        ...
        self.conv = nn.Conv1d(self.d_l, 128, kernel_size=1, stride=1, bias=False)
    ...
    def forward(
        ...
        output_h = 1 * self.att(output_h, hn, tool) + 0 * self.conv(v_t[:, -1, :].transpose(0, 1)).transpose(0, 1)
        # ib start
